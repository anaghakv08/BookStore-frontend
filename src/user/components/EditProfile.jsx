...
function EditProfile() {
    ...
    const [offCanavs, setOffCanvas] = useState(false)
        ...
        if (!username || !password || !confirmPassword || !bio) {
            ...
            if (password != confirmPassword) {
                ...
                if (preview) {
                    ...
                    const result = await updateUserProfileAPI({ username, password, confirmPassword, bio, role, profile: existingProfile }, reqHeader)
                    console.log(result);
    ...
    return (
        <>
            <button onClick={() => setOffCanvas(true)} className='flex px-4 py-3 font-bold border border-blue-200 text-blue-600 '>
                <FaRegEdit className='mt-1 me-2' /> Edit</button>
                ...
                <div className='bg-white h-full w-90 z-50 fixed top-0 left-0'>
                    ...
                    <div className='flex justify-center items-center flex-col my-5'>
                        <label htmlFor="profilePic">
                            <input onChange={(e) => handleImageUpload(e)} type="file" id='profilePic' style={{ display: "none" }} />


                            {existingProfile == "" ?
                                <img src={preview ? preview : "https://cdn.pixabay.com/photo/2023/02/18/11/00/icon-7797704_640.png"}
                                    alt="" style={{ height: "150px", width: "150px", borderRadius: "50%" }} />
                                :

                                <img src={preview ? preview : `${SERVERURL}/imguploads/${existingProfile}`}
                                    alt="" style={{ height: "150px", width: "150px", borderRadius: "50%" }} />
                            }
                        </label>
                    </div>
